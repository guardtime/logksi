...
@test "sign signed.logsig" {
	run ./src/logksi sign test/out/signed -dd
	[ "$status" -eq 0 ]
...
@test "sign signed2.logsig to output signed3.logsig" {
	run ./src/logksi sign test/out/signed2 -o test/out/signed3.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "sign from standard input" {
	run bash -c "cat test/out/unsigned.logsig | ./src/logksi sign --sig-from-stdin -o test/out/signed_from_stdin.logsig -dd"
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "sign signed2.logsig to stdout" {
	run bash -c "./src/logksi sign test/out/signed2 -dd -o - > test/out/signed_stdout.logsig"
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "sign unsigned.logsig" {
	run ./src/logksi sign test/out/unsigned -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
