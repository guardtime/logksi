...
@test "Verify output with debug level 2. Multiple blocks. Missing hshes. " {
	run src/logksi verify test/resource/logs_and_signatures/log_repaired --ignore-desc-block-time -dd
	[ "$status" -eq 0 ]
...
@test "Previous block is more recent than next block with debug level 1." {
	run ./src/logksi verify -d test/resource/logs_and_signatures/log_repaired
	[ "$status" -eq 6 ]
...
@test "Previous block is more recent than next block with debug level 2." {
	run ./src/logksi verify -dd test/resource/logs_and_signatures/log_repaired
	[ "$status" -eq 6 ]
