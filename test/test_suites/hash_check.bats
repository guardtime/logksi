...
@test "insert missing hashes" {
	run ./src/logksi sign test/out/all_hashes -o test/out/no_missing_hashes.logsig -ddd --insert-missing-hashes
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}
...
@test "verify record hash out of block" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_out_of_block.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify record hashes not stored in first block" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hashes_not_stored_in_first_block.logsig -ddd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify record hashes not stored in second block" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hashes_not_stored_in_second_block.logsig -ddd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify tree hashes not stored in first block" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hashes_not_stored_in_first_block.logsig -ddd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify tree hashes not stored in second block" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hashes_not_stored_in_second_block.logsig -ddd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify record hash missing for first record" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_missing_for_first_record.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify record hash missing for second record" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_missing_for_second_record.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify record hash missing for last record" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_missing_for_last_record.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify tree hash missing for first record" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_missing_for_first_record.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify tree hash 1 missing for second record" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_1_missing_for_second_record.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify tree hash 2 missing for second record" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_2_missing_for_second_record.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify tree hash missing for last record" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_missing_for_last_record.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify hashes missing for first record" {
	run ./src/logksi verify test/out/all_hashes test/out/hashes_missing_for_first_record.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify hashes missing for last record" {
	run ./src/logksi verify test/out/all_hashes test/out/hashes_missing_for_last_record.logsig -ddd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Block no. 1: missing record hash for logline no. 3." ]]
	[[ "$output" =~ "Log signature verification failed." ]]
}
...
@test "verify tree hash missing for metarecord in empty block" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_missing_for_metarecord_in_empty_block.logsig -ddd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}
...
@test "verify record hash too many" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_too_many.logsig -ddd
	[ "$status" -ne 0 ]
...
@test "verify tree hashes final all present" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hashes_final_all_present.logsig -ddd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Block no.  13: interpreting tree hash no.  27 as a final hash... ok." ]]
