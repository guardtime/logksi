...
cp -r test/resource/logsignatures/record_hash_missing_for_metarecord_in_empty_block.logsig test/out
cp -r test/resource/logsignatures/tree_hash_missing_for_metarecord_in_empty_block.logsig test/out
...
@test "verify all hashes" {
	run ./src/logksi verify test/out/all_hashes -dd
	[ "$status" -eq 0 ]
...
@test "insert missing hashes" {
	run ./src/logksi sign test/out/all_hashes -o test/out/no_missing_hashes.logsig -dd --insert-missing-hashes
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify no missing hashes" {
	run ./src/logksi verify test/out/all_hashes test/out/no_missing_hashes.logsig -dd
	[ "$status" -eq 0 ]
...
@test "verify record hash out of block" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_out_of_block.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hash out of block" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_out_of_block.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify record hashes not stored in first block" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hashes_not_stored_in_first_block.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify record hashes not stored in second block" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hashes_not_stored_in_second_block.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify tree hashes not stored in first block" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hashes_not_stored_in_first_block.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify tree hashes not stored in second block" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hashes_not_stored_in_second_block.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify record hash missing for first record" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_missing_for_first_record.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify record hash missing for second record" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_missing_for_second_record.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify record hash missing for last record" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_missing_for_last_record.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hash missing for first record" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_missing_for_first_record.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hash 1 missing for second record" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_1_missing_for_second_record.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hash 2 missing for second record" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_2_missing_for_second_record.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hash missing for last record" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_missing_for_last_record.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify hashes missing for first record" {
	run ./src/logksi verify test/out/all_hashes test/out/hashes_missing_for_first_record.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify hashes missing for last record" {
	run ./src/logksi verify test/out/all_hashes test/out/hashes_missing_for_last_record.logsig -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Block no. 1: missing record hash for logline no. 3." ]]
	[[ "$output" =~ "Log signature verification failed." ]]
}
...
@test "verify record hash missing for metarecord in empty block" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_missing_for_metarecord_in_empty_block.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify tree hash missing for metarecord in empty block" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_missing_for_metarecord_in_empty_block.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
}

@test "verify record hash missing for metarecord" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_missing_for_metarecord.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hash missing for metarecord" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_missing_for_metarecord.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify record hash too many" {
	run ./src/logksi verify test/out/all_hashes test/out/record_hash_too_many.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hash too many perfect tree" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_too_many_perfect_tree.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hash final" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_final.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Block no.   1: interpreting tree hash no.   5 as a final hash... ok." ]]
	[[ "$output" =~ "Block no.   1: all final tree hashes are present." ]]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "verify tree hash final wrong" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hash_final_wrong.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hashes final too many" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hashes_final_too_many.logsig -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Block no.   1: interpreting tree hash no.   5 as a final hash... ok." ]]
...
@test "verify tree hashes final too few" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hashes_final_too_few.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify tree hashes final all present" {
	run ./src/logksi verify test/out/all_hashes test/out/tree_hashes_final_all_present.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Block no.  13: interpreting tree hash no.  27 as a final hash... ok." ]]
