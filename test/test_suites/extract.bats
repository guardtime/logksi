...
@test "extract record 1" {
	run ./src/logksi extract test/out/extract.base -r 1 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.base -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, specify log records file" {
	run ./src/logksi extract test/out/extract.base.1 --out-log test/out/extract.user.1.excerpt -r 1 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.user.1.excerpt test/out/extract.base.1.excerpt.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, specify integrity proof file" {
	run ./src/logksi extract test/out/extract.base.2 --out-proof test/out/extract.user.2.excerpt.logsig -r 1 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.base.2.excerpt test/out/extract.user.2.excerpt.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, specify both output files" {
	run ./src/logksi extract test/out/extract.base.3 --out-log test/out/extract.user.3.excerpt --out-proof test/out/extract.user.3.excerpt.logsig -r 1 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.user.3.excerpt -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, specify output" {
	run ./src/logksi extract test/out/extract.base.4 -o test/out/extract.user.4 -r 1 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.user.4.excerpt -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, specify output, override log records" {
	run ./src/logksi extract test/out/extract.base.5 -o test/out/extract.user.5 --out-log test/out/extract.user.5.1 -r 1 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.user.5.1 test/out/extract.user.5.excerpt.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, specify output, override integrity proof" {
	run ./src/logksi extract test/out/extract.base.6 -o test/out/extract.user.6 --out-proof test/out/extract.user.6.1 -r 1 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.user.6.excerpt test/out/extract.user.6.1 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, redirect log records to stdout" {
	run bash -c "./src/logksi extract test/out/extract.base.7 --out-log - -r 1 -dd > test/out/extract.user.7.stdout"
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.user.7.stdout test/out/extract.base.7.excerpt.logsig -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, redirect integrity proof to stdout" {
	run bash -c "./src/logksi extract test/out/extract.base.8 --out-proof - -r 1 -dd > test/out/extract.user.8.stdout"
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.base.8.excerpt test/out/extract.user.8.stdout -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, read log file from stdin" {
	...
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.user.9.excerpt -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract record 1, read signature file from stdin" {
	run bash -c "cat test/out/extract.base.10.logsig | ./src/logksi extract test/out/extract.base.10 --sig-from-stdin -r 1 -dd"
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.base.10.excerpt -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "verify record 1 too long" {
	run ./src/logksi verify test/resource/logfiles/r1_too_long.excerpt test/out/extract.base.excerpt.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify proof wrong record hash" {
	run ./src/logksi verify test/out/extract.base.excerpt test/resource/logsignatures/proof_wrong_record_hash.excerpt.logsig -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Block no. 1: record hashes not equal." ]]
	[[ "$output" =~ "Log signature verification failed." ]]
}
...
@test "verify proof wrong sibling hash 1" {
	run ./src/logksi verify test/out/extract.base.excerpt test/resource/logsignatures/proof_wrong_sibling_hash_1.excerpt.logsig -dd
	[ "$status" -ne 0 ]
...
@test "verify proof wrong sibling hash 3" {
	run ./src/logksi verify test/out/extract.base.excerpt test/resource/logsignatures/proof_wrong_sibling_hash_3.excerpt.logsig -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Block no. 1: root hashes not equal." ]]
	[[ "$output" =~ "Log signature verification failed." ]]
}
...
@test "verify proof wrong level correction 1" {
	run ./src/logksi verify test/out/extract.base.excerpt test/resource/logsignatures/proof_wrong_level_correction_1.excerpt.logsig -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Block no. 1: root hashes not equal." ]]
	[[ "$output" =~ "Log signature verification failed." ]]
}
...
@test "verify proof wrong level correction 2" {
	run ./src/logksi verify test/out/extract.base.excerpt test/resource/logsignatures/proof_wrong_level_correction_2.excerpt.logsig -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Block no. 1: root hashes not equal." ]]
	[[ "$output" =~ "Log signature verification failed." ]]
}
...
@test "verify proof wrong link direction" {
	run ./src/logksi verify test/out/extract.base.excerpt test/resource/logsignatures/proof_wrong_link_direction.excerpt.logsig -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Block no. 1: root hashes not equal." ]]
	[[ "$output" =~ "Log signature verification failed." ]]
}
...
@test "extract last (log)record" {
	run ./src/logksi extract test/out/extract.base -r 1414 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.base.excerpt -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract range over three blocks" {
	run ./src/logksi extract test/out/extract.base -r 3-7 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.base.excerpt -dd
	[ "$status" -eq 0 ]
...
@test "extract range over three blocks, alternative range definition 1" {
	run ./src/logksi extract test/out/extract.base -r 3-5,6,7 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.base.excerpt -dd
	[ "$status" -eq 0 ]
...
@test "extract range over three blocks, alternative range definition 2" {
	run ./src/logksi extract test/out/extract.base -r 3,4,5-7 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.base.excerpt -dd
	[ "$status" -eq 0 ]
...
@test "extract range over three blocks, isolated positions" {
	run ./src/logksi extract test/out/extract.base -r 3,6,9 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/extract.base.excerpt -dd
	[ "$status" -eq 0 ]
...
@test "extract records from non-extended legacy.gtsig" {
	run ./src/logksi verify test/out/legacy_extract -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Warning: RFC3161 timestamp(s) found in log signature." ]]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi extract test/out/legacy_extract -r 1-20 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Warning: RFC3161 timestamp(s) found in log signature." ]]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/legacy_extract.excerpt -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
...
@test "extract records from extended legacy.gtsig" {
	run ./src/logksi extend test/out/legacy_extract --enable-rfc3161-conversion -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi extract test/out/legacy_extract -r 21-204 -dd
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok." ]]
	run ./src/logksi verify test/out/legacy_extract.excerpt -dd
	[ "$status" -eq 0 ]
...
@test "attempt to extract a list that contains positions out of range" {
	run ./src/logksi extract test/out/extract.base -r 1415 -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Extract position 1415 out of range - not enough loglines." ]]
	run ./src/logksi extract test/out/extract.base -r 1413-1416 -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Extract position 1415 out of range - not enough loglines." ]]
	run ./src/logksi extract test/out/extract.base -r 1413,1414,1415,1416 -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Extract position 1415 out of range - not enough loglines." ]]
	run ./src/logksi extract test/out/extract.base -r 1-999999999999999 -dd
	[ "$status" -ne 0 ]
	[[ "$output" =~ "Error: Extract position 1415 out of range - not enough loglines." ]]
	run ./src/logksi extract test/out/legacy_extract -r 1-205 -dd
	[ "$status" -ne 0 ]
