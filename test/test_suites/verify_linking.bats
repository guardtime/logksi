...
@test "verify inter-linking of two sequential log signatures by saving temporary hash imprint to file." {
	run ./src/logksi verify test/resource/interlink/ok-testlog-interlink-1 -dd --output-hash test/out/ok-testlog-interlink-1-output-hash
	[ "$status" -eq 0 ]
	[[ "$output" =~ "Finalizing log signature... ok" ]]

	run ./src/logksi verify test/resource/interlink/ok-testlog-interlink-2 -dd --input-hash test/out/ok-testlog-interlink-1-output-hash
	[ "$status" -eq 0 ]
	[[ "$output" =~ (Block no).*(1).*(verifying inter-linking input hash... ok).*(Finalizing log signature... ok) ]]
}

@test "verify inter-linking of two sequential log signatures by passing previous leaf hash imprint value via stdout." {
	run bash -c "./src/logksi verify test/resource/interlink/ok-testlog-interlink-1 -dd --output-hash - | ./src/logksi verify test/resource/interlink/ok-testlog-interlink-2 -dd --input-hash -"
	[ "$status" -eq 0 ]
	[[ "$output" =~ (Block no).*(1).*(verifying inter-linking input hash... ok).*(Finalizing log signature... ok) ]]
}
...
@test "verify inter-linking automatically by giving 2 log files after --." {
	run src/logksi verify -dd -- test/resource/interlink/ok-testlog-interlink-1 test/resource/interlink/ok-testlog-interlink-2
	[ "$status" -eq 0 ]
...
@test "verify inter-linking automatically by giving 2 log files after --. Check --input-hash" {
	run src/logksi verify -dd --input-hash sha2-256:a558295ae8da8cf4e2b13a34289d2a17676821f14e0792ac1098d27d9bea5fc9 -- test/resource/interlink/ok-testlog-interlink-1 test/resource/interlink/ok-testlog-interlink-2
	[ "$status" -eq 0 ]
...
@test "verify inter-linking automatically by giving 2 log files after --. Check output hash (must match to last log and log signature)." {
	run src/logksi verify -dd --output-hash - -- test/resource/interlink/ok-testlog-interlink-1 test/resource/interlink/ok-testlog-interlink-2
	[ "$status" -eq 0 ]
...
@test "verify inter-linking automatically by giving multiple log files with wildcard after --." {
	run bash -c "./src/logksi verify -dd -- test/resource/interlink/ok-testlog-interlink-[12]"
	[ "$status" -eq 0 ]
	[[ "$output" =~ (Log file.*ok-testlog-interlink-1).*(Finalizing log signature... ok).*(Log file.*ok-testlog-interlink-2).*(Block no).*(1).*(verifying inter-linking input hash... ok).*(Finalizing log signature... ok) ]]
}
...
@test "verify inter-linking where first log is resigned." {
	run src/logksi verify -dd -- test/resource/interlink/ok-testlog-interlink-resigned-1 test/resource/interlink/ok-testlog-interlink-2
	[ "$status" -eq 6 ]
