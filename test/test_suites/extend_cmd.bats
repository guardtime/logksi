...
@test "Extend CDM test: try to extend signature from stdin and from command line simultaneously" {
...
@test "Extend CDM test: try to use invalid stdout combination" {
...
@test "Extend CDM test: try to use only one not existing log file"  {
	run src/logksi extend i_do_not_exist -o test/out/dummy.ksig -d
	[ "$status" -eq 3 ]
	[[ "$output" =~ (File does not exist).*(Parameter).*(--input).*(i_do_not_exist) ]]
}

@test "Extend CDM test: try to use two not existing input files"  {
...
@test "Extend CDM test: try to use one unexpected but existing extra input file"  {
...
@test "Extend CDM test: try to use invalid publication string: Invalid character" {
...
# TODO: Fix...
#@test "Extend CDM test: try to use invalid publication string: Too short" {
...
#@test "Extend CDM test: try to use invalid publication string: Too long" {
...
@test "Extend CDM test: try to use invalid certificate constraints: Invalid constraints format" {
...
@test "Extend CDM test: try to use invalid certificate constraints: Invalid constraints OID" {
